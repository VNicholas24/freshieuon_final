import Header from "@/components/admin/header";
import classes from "./index.module.css";
import { useRouter } from "next/router";
import { CircularProgress, Input, TextField } from "@mui/material";
import SideBar from "@/components/admin/sideBar/sideBar";
import { FunctionComponent, useCallback, useEffect, useState } from "react";
import { initializeApp } from "firebase/app";

import {
  addDoc,
  collection,
  doc,
  getDoc,
  getFirestore,
  setDoc,
  serverTimestamp,
  getDocs,
  updateDoc,
  deleteDoc,
  Timestamp,
} from "firebase/firestore";
import Button from "@mui/material/Button";
import dynamic from "next/dynamic";
import "easymde/dist/easymde.min.css";
import HeaderEdit from "@/components/admin/pagesContent/hearderEdit";
import { Modal } from "@mui/material";
import {
  getStorage,
  ref,
  uploadString,
  getDownloadURL,
  uploadBytes,
  deleteObject,
} from "firebase/storage";
import FoodWheel from "@/components/admin/getting-around-sg/foodWheel";
import FoodItem from "@/components/admin/getting-around-sg/foodWheel";
import RenderedSection from "@/components/admin/preparation-guide/toDoRenderedSection";

type Paragraph = {
  id: string;
  title: string;
  description: string;
};

type Action = {
  id: string;
  name: string;
  description: string;
  isCompleted: boolean;
};

interface Todo {
  id: string;
  category: string;
  imageUrl: string;
  name: string;
  progress: string;
  slug: string;
  actions: Action[];
  paragraphs: Paragraph[];
}
export default function DetailPage() {
  const router = useRouter();
  const currentUrl = router.asPath;
  const urlArray: string[] = currentUrl.split("/");
  const [todo, setTodo] = useState<Todo>({
    id: "",
    category: "",
    imageUrl: "",
    name: "",
    progress: "",
    slug: "",
    actions: [],
    paragraphs: [],
  });
  const blogTitle = urlArray[4];
  urlArray[urlArray.length - 1] = blogTitle;
  const [isLoading, setIsLoading] = useState(true);
  const [text, settext] = useState(" ");
  const [slug, setslug] = useState("");
  const [title, settitle] = useState("");
  const timestamp = serverTimestamp();
  const [bannerTitle, setBannerTitle] = useState("");
  const [bannerImageUrl, setBannerImageUrl] = useState("");
  const [bannerDescription, setBannerDescription] = useState("");
  const [errorstates, seterrorstates] = useState({
    sameslug: false,
    emptytext: false,
  });
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalMessage, setModalMessage] = useState("");
  const [postedBy, setPostedBy] = useState("");
  const [createdAt, setCreatedAt] = useState(Timestamp.now);
  const [isPublishedGlobally, setisPublishedGlobally] = useState(false);
  const [isImageUploaded, setIsImageUploaded] = useState(false);
  const [paragraphs, setParagraphs] = useState<Paragraph[]>([]);
  const [actions, setActions] = useState<Action[]>([]);
  const [uploadedIconImage, setUploadedIconImage] = useState(todo.imageUrl);
  const [isIconImageUploaded, setIsIconImageUploaded] = useState(false);
  const [imageError, setImageError] = useState("");
  const [imageIconError, setImageIconError] = useState("");

  const fetchTodo = async () => {
    if (blogTitle) {
      const todoDocRef = doc(
        getFirestore(),
        "preparation-guide-todos",
        blogTitle
      );
      const todoDocSnap = await getDoc(todoDocRef);

      if (todoDocSnap.exists()) {
        const todoData = todoDocSnap.data();
        const todo: Todo = {
          id: todoDocSnap.id,
          category: todoData.category,
          imageUrl: todoData.imageUrl,
          name: todoData.name,
          progress: todoData.progress,
          slug: todoData.slug,
          actions: [],
          paragraphs: [],
        };

        const actionsQuery = collection(
          getFirestore(),
          "preparation-guide-todos",
          blogTitle,
          "actions"
        );
        const actionsSnapshot = await getDocs(actionsQuery);

        actionsSnapshot.forEach((actionDoc) => {
          const actionData = actionDoc.data();
          const action: Action = {
            id: actionDoc.id,
            name: actionData.name,
            description: actionData.description,
            isCompleted: actionData.isCompleted,
          };
          todo.actions.push(action);
        });

        const paragraphsQuery = collection(
          getFirestore(),
          "preparation-guide-todos",
          blogTitle,
          "paragraphs"
        );
        const paragraphsSnapshot = await getDocs(paragraphsQuery);

        paragraphsSnapshot.forEach((paragraphDoc) => {
          const paragraphData = paragraphDoc.data();
          const paragraph: Paragraph = {
            id: paragraphDoc.id,
            title: paragraphData.title,
            description: paragraphData.description,
          };
          todo.paragraphs.push(paragraph);
        });

        setTodo(todo);
        setUploadedIconImage(todo.imageUrl);
      } else {
        console.log("Todo does not exist");
      }
    }
    setIsLoading(false);
  };

  useEffect(() => {
    const timeOutId = setTimeout(async () => {
      fetchTodo();
    }, 1000);
    return () => clearTimeout(timeOutId);
  }, [blogTitle]);

  const fetchPage = useCallback(async () => {
    const docs = await getDocs(
      collection(getFirestore(), "preparation-guide-detailed-page-header")
    );
    docs.forEach((doc) => {
      if (doc.id == blogTitle) {
        setBannerTitle(doc.data().title);
        setBannerDescription(doc.data().description);
        setBannerImageUrl(doc.data().imageUrl);
        settitle((title) => doc.data().title);
        setisPublishedGlobally(doc.data().isPublishedGlobally);
        setPostedBy(doc.data().postedBy);
        setCreatedAt(doc.data().createdAt);
      }
    });
  }, [blogTitle]);

  useEffect(() => {
    const timeOutId = setTimeout(async () => {
      setIsLoading(true);
      fetchPage().then(() => setIsLoading(false));
    }, 1000);
    return () => clearTimeout(timeOutId);
  }, [fetchPage, blogTitle]);

  const postPage = async () => {
    if (text === "") {
      seterrorstates({ ...errorstates, emptytext: true });
    } else {
      const docCurRef = doc(
        getFirestore(),
        "preparation-guide-detailed-page-header",
        blogTitle
      );
      const docCurSnap = await getDoc(docCurRef);
      try {
        if (docCurSnap.exists()) {
          await updateDoc(docCurRef, {
            title: title,
            text: text,
            isPublishedGlobally: isPublishedGlobally,
          });
          setModalMessage("Page successfully updated.");
        } else {
          await setDoc(docCurRef, {
            title: title,
            text: text,
            createdAt: timestamp,
            postedBy: "Admin",
            isPublishedGlobally: true,
          });
          setModalMessage("Page successfully added.");
        }

        // Update banner image
        const storage = getStorage();
        const storageRef = ref(
          storage,
          `preparation-guide/-detailed-page-${blogTitle}`
        );

        if (isIconImageUploaded) {
          await uploadString(storageRef, uploadedIconImage, "data_url");
        }
        const imageUrl = isIconImageUploaded
          ? await getDownloadURL(storageRef)
          : todo.imageUrl;

        const docBannerRef = doc(
          getFirestore(),
          "preparation-guide-todos",
          blogTitle
        );

        const docBannerSnap = await getDoc(docBannerRef);
        if (docBannerSnap.exists()) {
          await updateDoc(docBannerRef, {
            imageUrl: imageUrl,
          });
        }

        setIsModalOpen(true);
      } catch (error) {
        console.log(error);
        setModalMessage("An error occurred while saving the page.");
        setIsModalOpen(true);
      }
    }
  };

  const postPagePreview = async () => {};

  const postHeader = async () => {
    const storage = getStorage();
    const storageRef = ref(
      storage,
      `preparation-guide/-detailed-page-header-${blogTitle}`
    );

    try {
      if (isImageUploaded) {
        await uploadString(storageRef, bannerImageUrl, "data_url");
      }
      const imageUrl = isImageUploaded
        ? await getDownloadURL(storageRef)
        : bannerImageUrl;
      const docRef = doc(
        getFirestore(),
        "preparation-guide-detailed-page-header",
        blogTitle
      );
      const docSnap = await getDoc(docRef);
      const curDocRef = doc(
        getFirestore(),
        "preparation-guide-detailed-page-header",
        blogTitle
      );
      const curDocSnap = await getDoc(curDocRef);
      if (curDocSnap.exists()) {
        // Delete the existing document if the slug is not the same
        await updateDoc(docRef, {
          title: bannerTitle,
          description: bannerDescription,
          imageUrl: imageUrl,
        });
        setModalMessage("Page successfully updated.");
      } else {
        await setDoc(docRef, {
          title: bannerTitle,
          description: bannerDescription,
          imageUrl,
          slug: blogTitle,
        });
        setModalMessage("Page successfully added.");
      }

      setIsModalOpen(true);
    } catch (error) {
      console.log(error);
      setModalMessage("An error occurred while saving the page.");
      setIsModalOpen(true);
    }
  };

  const postHeaderPreview = async () => {};

  const onTitleChange = (title: string) => {
    settitle(title);
  };

  const handleIconImageUpload = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = event.target.files?.[0];
    if (file) {
      const allowedTypes = ["image/jpeg", "image/png", "image/gif"];
      if (allowedTypes.includes(file.type)) {
        const reader = new FileReader();
        reader.onload = (e) => {
          if (e.target?.result) {
            const imageUrl = e.target.result.toString();
            setUploadedIconImage(imageUrl);
            setIsIconImageUploaded(true);
            setImageIconError("");
          }
        };
        reader.readAsDataURL(file);
      } else {
        setImageIconError("Invalid file type. Please upload an image.");
      }
    }
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setModalMessage("");
  };

  return (
    <div>
      {isLoading ? (
        <div className={classes.loading}>
          <CircularProgress /> {/* Display the spinner */}
          <p>Loading...</p>
        </div>
      ) : title === "" ? (
        <p>Page not found</p>
      ) : (
        <div className={classes.container}>
          <Header id={todo.id} blogTitle={blogTitle} urlArray={urlArray} category={""} />
          <div className={classes.flexContainer}>
            <div className={classes.bodyContainer}>
              <div className={classes.upperSecContainer}>
                <h3 className={classes.sectionTitle}>Title</h3>
                <TextField
                  InputProps={{ className: classes.input }}
                  sx={{ width: "600px" }}
                  size="small"
                  id="title"
                  variant="outlined"
                  onChange={(e) => onTitleChange(e.target.value)}
                  value={title}
                />
                <h3 className={classes.sectionTitle}>Slug</h3>
                <TextField
                  disabled
                  InputProps={{ className: classes.input }}
                  sx={{ width: "600px" }}
                  size="small"
                  id="slug"
                  variant="outlined"
                  value={todo.slug}
                  onChange={(e) => {
                    setslug(e.target.value);
                    seterrorstates({ ...errorstates, sameslug: false });
                  }}
                  helperText={errorstates.sameslug ? "Slug already exists" : ""}
                />

                <h3 className={`${classes.sectionTitle}`}>Upload Image :</h3>
                <Input
                  type="file"
                  id="image"
                  onChange={handleIconImageUpload}
                />
                {imageError && (
                  <p style={{ color: "red", textAlign: "end" }}>{imageError}</p>
                )}
                <div className={classes.textFieldContainer}>
                  <img
                    src={uploadedIconImage}
                    alt="Banner Image"
                    className={classes.image}
                  />
                </div>
                <h3 className={classes.sectionTitle}>Header</h3>
              </div>
              <HeaderEdit
                headerTitle={bannerTitle}
                headerDescription={bannerDescription}
                headerImageUrl={bannerImageUrl}
                setBannerTitle={setBannerTitle}
                setBannerImageUrl={setBannerImageUrl}
                setBannerDescription={setBannerDescription}
                setIsImageUploaded={setIsImageUploaded}
              />
              <RenderedSection todo={todo}></RenderedSection>
              <Modal
                open={isModalOpen}
                onClose={closeModal}
                className={classes.modal}
              >
                <div className={classes.modalContainer}>
                  <h2>{modalMessage}</h2>
                  <Button variant="contained" onClick={closeModal}>
                    Close
                  </Button>
                </div>
              </Modal>
            </div>
            <div className={classes.sideBar}>
              <SideBar
                postPage={postPage}
                postPagePreview={postPagePreview}
                postHeader={postHeader}
                postHeaderPreview={postHeaderPreview}
                isPublished={isPublishedGlobally}
                postedBy={postedBy}
                postedDate={createdAt}
                setIsPublished={setisPublishedGlobally}
              ></SideBar>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
