...
export default function DetailPage() {
  ...
  const fetchPage = useCallback(async () => {
    const querySnapshot = await getDocs(
      collection(getFirestore(), "courses-blog")
    );
    ...
    const docs = await getDocs(
      collection(getFirestore(), "courses-blog-header")
    );
    docs.forEach((doc) => {
      if (doc.id == blogTitle) {
  ...
  const postPage = async () => {
    if (text === "") {
      ...
      const docRef = doc(getFirestore(), "courses-blog", slug);
      const docCurRef = doc(getFirestore(), "courses-blog", blogTitle);
        ...
        if (docCurSnap.exists()) {
          if (slug != blogTitle) {
            ...
            await setDoc(docRef, {
              ...
              router.replace(`/admin/pages/courses/blogTitle?=${slug}`);
            });
  ...
  const postPagePreview = async () => {
    if (text === "") {
      ...
      const docRef = doc(getFirestore(), "courses-blog-preview", slug);
      const docCurRef = doc(
        getFirestore(),
        "courses-blog-preview",
        blogTitle
      );
      const docSnap = await getDoc(docRef);
      const docCurSnap = await getDoc(docCurRef);
  ...
  const postHeader = async () => {
    ...
    const storageRef = ref(
      storage,
      `courses/courses-${slug}-header-banner/`
    );
    ...
    try {
      ...
      const imageUrl = isImageUploaded
        ...
        : bannerImageUrl;
      const docRef = doc(getFirestore(), "courses-blog-header", slug);
      const docSnap = await getDoc(docRef);
      const curDocRef = doc(
        getFirestore(),
        "courses-blog-header",
  ...
  const postHeaderPreview = async () => {
    ...
    const storageRef = ref(
      storage,
      `courses/courses-${slug}-header-banner-preview/`
    );
      ...
      if (curDocSnap.exists()) {
        ...
        if (slug != blogTitle) {
          ...
          await setDoc(docRef, {
            ...
            router.replace(`/courses/${slug}/preview`);
          });
        } else {
          await updateDoc(docRef, {
            title: bannerTitle,
