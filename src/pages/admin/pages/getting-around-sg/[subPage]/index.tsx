...
import SubPageSection from "@/components/admin/getting-around-sg/subPageSection";
...
export default function DetailPage() {
  ...
  const postPage = async () => {
    if (text === "") {
      ...
      const docRef = doc(
        getFirestore(),
        "getting-around-sub-page-header",
        slug
      );
      const docCurRef = doc(
        getFirestore(),
  ...
  const postHeader = async () => {
    ...
    const storageRef = ref(
      storage,
      `getting-around-sg/sub-page-${slug}-header-banner/`
    );
