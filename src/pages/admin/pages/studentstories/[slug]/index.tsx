...
const Edit: FunctionComponent<editProps> = () => {
    ...
    const [header, setheader] = useState(null as File | null);
    ...
    return (
        <>
            ...
            <div style={{ backgroundColor: "#EDF6FF", padding: "50px" }}>
                <h2 style={{ margin: 0, paddingBottom: "20px", color: "#6E94AF" }}>{data.title}</h2>
                <p style={{ margin: 0, color: "#000000" }}>Pages {'>'} {data.title} {'>'}
                    <a href={`/studentstories/${slug}`} style={{ color: "#0093FF", textDecoration: "underline" }}>
                        {data.title}
                    </a>
                </p>
            </div>
            <Grid container spacing={2} style={{ backgroundColor: '#EBEEF0' }}>
                <Grid item xs={12} xl={9} style={{ paddingLeft: '30px' }}>
                    ...
                    <div style={{ backgroundColor: "#FFFFFF", padding: '30px', borderRadius: '4px', width: "100%" }}>
                        <div style={{ backgroundColor: "#DCE2EA", borderRadius: '4px', padding: '30px' }}>
                            <Button style={{ margin: '0 0 10px 0', color: "#000000" }} variant="text" component="label" endIcon={<DriveFolderUploadIcon />}>Change profile photo<input hidden accept="image/*" multiple type="file" onChange={(e) => {
