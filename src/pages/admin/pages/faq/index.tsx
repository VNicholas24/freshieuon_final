...
import AddFaqSection from "@/components/website/faq/addFaq";
...
export default function DetailPage() {
  ...
  const [faq, setFaq] = useState<Faq>({ question: "", answer: "faq" });
  ...
  const fetchPage = useCallback(async () => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (docSlug == faqTitle) {
        ...
        setFaq({ question: doc.data().question, answer: doc.data().answer });
        setisPublishedGlobally(doc.data().isPublishedGlobally);
        setPostedBy(doc.data().postedBy);
        setCreatedAt(doc.data().createdAt);
      }
    });
    const docs = await getDocs(collection(getFirestore(), "faq-header"));
    docs.forEach((doc) => {
      if (doc.id == faqTitle) {
  ...
  const postPage = async () => {
    if (text === "") {
      ...
      const docRef = doc(getFirestore(), "faq-header", slug);
      const docCurRef = doc(getFirestore(), "faq-header", faqTitle);
  ...
  const postHeader = async () => {
    ...
    const storageRef = ref(storage, `faq/faq-${slug}-header/`);
    ...
    try {
      ...
      const docRef = doc(getFirestore(), "faq-header", slug);
      const docSnap = await getDoc(docRef);
      const curDocRef = doc(getFirestore(), "faq-header", faqTitle);
