...
export default function DetailPage() {
  ...
  const postPagePreview = async () => {
    if (text === "") {
      ...
      const docRef = doc(getFirestore(), "accommodation-blog-preview", slug);
      const docCurRef = doc(
        getFirestore(),
        "accommodation-blog-preview",
        blogTitle
      );
      const docSnap = await getDoc(docRef);
      const docCurSnap = await getDoc(docCurRef);
  ...
  const postHeader = async () => {
    ...
    const storageRef = ref(
      storage,
      `accommodation/accommodation-${slug}-header-banner/`
    );
    ...
    try {
      ...
      const imageUrl = isImageUploaded
        ...
        : bannerImageUrl;
      const docRef = doc(getFirestore(), "accommodation-blog-header", slug);
      const docSnap = await getDoc(docRef);
      const curDocRef = doc(
        getFirestore(),
        "accommodation-blog-header",
        blogTitle
      );
  ...
  const postHeaderPreview = async () => {
    ...
    const storageRef = ref(
      storage,
      `accommodation/accommodation-${slug}-header-banner-preview/`
    );
    ...
    try {
      ...
      const docRef = doc(
        getFirestore(),
        "accommodation-blog-header-preview",
        slug
      );
      const docSnap = await getDoc(docRef);
      const curDocRef = doc(
      ...
      if (curDocSnap.exists()) {
        ...
        if (slug != blogTitle) {
          ...
          await setDoc(docRef, {
            ...
            router.replace(`/accommodation/${slug}/preview`);
          });
        } else {
          await updateDoc(docRef, {
            title: bannerTitle,
