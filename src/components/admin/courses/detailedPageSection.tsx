...
export default function DetailedPageSection() {
  ...
  const fetchPageSections = useCallback(async () => {
    const querySnapshot = await getDocs(
      collection(getFirestore(), "courses-page")
    );
  ...
  const handleDeleteSection = async (id: string) => {
    ...
    const docRef = doc(getFirestore(), "courses-page", id);
    ...
    try {
      ...
      const storageRef = ref(storage, `courses/courses-page-${id}`);
  ...
  const handleDeleteBlog = async (blogId: string) => {
    const docRef = doc(getFirestore(), "courses-blog", blogId);
            ...
            {blogs.map((blog, index) => (
              <div key={blog.id} className={classes.blogContainer}>
                <Link
                  ...
                  href={`/admin/pages/courses/blogTitle?=${blog.slug}`}
                >
